...
import { MapPin, Phone, ShieldCheck, Clock, MessageSquare, Navigation, ArrowUpRight } from "lucide-react";
...
export function ContactSection() {
  ...
  const mapEmbedUrl =
    "https://maps.google.com/maps?q=R.+%C3%89bano+Pereira,+60+-+Centro,+Curitiba+-+PR,+80410-240&t=&z=16&ie=UTF8&iwloc=&output=embed";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="font-body text-sm sm:text-base text-[var(--text-muted)] max-w-xl leading-relaxed">
            Sede no Centro de Curitiba/PR com estrutura privativa para atendimento presencial e suporte online para clientes em todo o Brasil.
          </p>
        </div>

        {/* Grid: Dados à Esquerda + Google Maps à Direita */}
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-stretch">
          {/* Coluna 1: Informações e Ações (Sem e-mail conforme determinação) */}
          <div ref={cardsColRef} className="lg:col-span-5 flex flex-col justify-between space-y-6 will-change-transform">
            <div className="space-y-4">
              {/* Card WhatsApp */}
              <div className="contact-info-card p-5 rounded-2xl bg-[var(--bg-secondary)]/70 border border-[var(--border-subtle)]/40 flex items-start gap-4 will-change-transform shadow-2xs hover:border-[#5B1D33] transition-colors">
                <div className="w-10 h-10 rounded-xl bg-[#5B1D33] text-white flex items-center justify-center flex-shrink-0 shadow-xs">
                  <Phone className="w-5 h-5" />
                </div>
                <div>
                  <span className="font-heading text-xs uppercase tracking-wider text-[#A64B6A] font-bold block mb-0.5">
                    WhatsApp & Telefone
                  </span>
                  ...
                  <p className="text-xs font-body text-[var(--text-muted)] mt-1">
                    Atendimento imediato para acolhimento inicial, esclarecimentos e agendamentos.
                  </p>
                </div>
              </div>

              {/* Card Endereço com Botão de Rota Traçada */}
              <div className="contact-info-card p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)]/35 flex items-start gap-4 will-change-transform shadow-2xs hover:border-[#5B1D33] transition-colors">
                <div className="w-10 h-10 rounded-xl bg-[var(--bg-secondary)] text-[#5B1D33] dark:text-[#E8C5D0] flex items-center justify-center flex-shrink-0 shadow-xs">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-body text-[var(--text-muted)] mt-1">
                    Centro • Curitiba - Paraná • CEP 80410-240
                  </p>
                  <div className="mt-3 pt-2.5 border-t border-[var(--border-subtle)]/25">
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 text-xs font-heading font-bold text-[#5B1D33] dark:text-[#E8C5D0] hover:text-[#A64B6A] transition-colors group/route"
                    >
                      <Navigation className="w-3.5 h-3.5 transition-transform group-hover/route:rotate-12 text-[#A64B6A]" />
              ...
              {/* Card Inscrição Profissional & OAB (Substitui o e-mail, conforme regra do usuário) */}
              <div className="contact-info-card p-5 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-subtle)]/35 flex items-start gap-4 will-change-transform shadow-2xs hover:border-[#5B1D33] transition-colors">
                <div className="w-10 h-10 rounded-xl bg-[var(--bg-secondary)] text-[#5B1D33] dark:text-[#E8C5D0] flex items-center justify-center flex-shrink-0 shadow-xs">
                ...
                <div>
                  <span className="font-heading text-xs uppercase tracking-wider text-[#A64B6A] font-bold block mb-0.5">
                    Inscrição Profissional & Sigilo
                  </span>
                  <p className="font-body text-sm font-semibold text-[var(--text-main)]">
                    {OFFICE_INFO.oab} • Direito de Família
                  </p>
                  <p className="text-xs font-body text-[var(--text-muted)] mt-1">
                    Atendimento ético e estritamente sigiloso, em conformidade com o CFOAB.
                  </p>
                </div>
              </div>
            ...
            <div className="pt-2">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="w-full btn-pill bg-[#5B1D33] hover:bg-[#360F1D] text-white py-3.5 gap-2 shadow-md text-sm sm:text-base cursor-pointer hover-lift transition-all flex items-center justify-center"
              >
          ...
          <div ref={mapColRef} className="lg:col-span-7 flex flex-col justify-between will-change-transform">
            <div className="relative w-full h-[380px] sm:h-[480px] lg:h-full min-h-[380px] rounded-2xl overflow-hidden border border-[var(--border-subtle)]/40 shadow-xs">
              <iframe
                title="Localização de Janaina Carvalho Advocacia em Curitiba PR"
                src={mapEmbedUrl}
                width="100%"
                height="100%"
                style={{ border: 0 }}
              ...
              <div className="absolute top-4 left-4 p-3 rounded-xl bg-white/95 dark:bg-[#151A1F]/95 backdrop-blur-md border border-[var(--border-subtle)]/30 text-xs shadow-md">
                ...
                <span className="text-[var(--text-muted)] font-body">
                  R. Ébano Pereira, 60 - Sl 2004 - Centro, Curitiba/PR
                </span>
              </div>
              ...
              <div className="absolute bottom-4 inset-x-4 sm:left-auto sm:right-4 p-2 sm:p-2.5 rounded-2xl bg-white/95 dark:bg-[#151A1F]/95 backdrop-blur-md border border-[var(--border-subtle)]/40 shadow-xl flex items-center justify-between sm:justify-start gap-3">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="btn-pill bg-[#5B1D33] hover:bg-[#360F1D] hover:scale-105 text-white py-2.5 px-4 text-xs font-semibold gap-2 shadow-md inline-flex items-center justify-center w-full sm:w-auto transition-all cursor-pointer"
                >
